...
public class JpaPurchaseRepositoryAdapter implements PurchaseRepository {
    ...
    public boolean existsByUserAndDate(UUID userId, LocalDate purchaseDate) {
        ...
        return purchaseOrderRepository.existsByUserIdAndPurchasedAtBetween(userId, startOfDay, endOfDay);
    }
    ...
    interface JpaMapper {
        ...
        default PurchaseOrderEntity.OrderStatus toEntityStatus(PurchaseOrder.OrderStatus status) {
            return PurchaseOrderEntity.OrderStatus.valueOf(status.name());
        }

        default PurchaseOrder.OrderStatus toDomainStatus(PurchaseOrderEntity.OrderStatus status) {
            return PurchaseOrder.OrderStatus.valueOf(status.name());
        }
    }
}
