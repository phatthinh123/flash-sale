...
public class OtpRepositoryAdapter implements OtpRepository {
    ...
    public Optional<OtpToken> findLatestUnusedByUserId(UUID userId) {
        return jpaOtpRepository.findFirstByUserIdAndUsedFalseOrderByCreatedAtDesc(userId).map(jpaMapper::toDomain);
    }

    @Override
    @Transactional
    public void markAsUsed(UUID otpTokenId) {
        jpaOtpRepository.findById(otpTokenId).ifPresent(entity -> {
            entity.setUsed(true);
